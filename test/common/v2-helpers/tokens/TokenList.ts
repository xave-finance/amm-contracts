...
export default class TokenList {
  ...
  async mint(rawParams: RawTokenMint): Promise<void> {
    ...
    await Promise.all(
      // params.flatMap(({ to, amount, from }: any) => this.tokens.map((token) => token.mint(to, amount, { from })))
      flatMap(params, ({ to, amount, from }: any) => this.tokens.map((token) => token.mint(to, amount, { from })))
    );
  }
  ...
  async approve(rawParams: RawTokenApproval): Promise<void> {
    ...
    await Promise.all(
      // params.flatMap(({ to, amount, from }: any) => this.tokens.map((token) => token.approve(to, amount, { from })))
      flatMap(params, ({ to, amount, from }: any) => this.tokens.map((token) => token.approve(to, amount, { from })))
    );
  }
