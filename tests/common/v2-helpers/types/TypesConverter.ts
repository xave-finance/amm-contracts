...
export default {
	toVaultDeployment(params: any): any {
	...
	toStablePoolDeployment(params: any): any {
		let {
			tokens,
			rateProviders,
		...
		return params.map((param, i) => {
			...
			const args = Object.assign(
				{},
				{
					symbol: `TK${i}`,
					name: `Token ${i}`,
					decimals: varyDecimals ? computeDecimalsFromIndex(i) : 18,
					from,
				},
				param
			)
	...
	toTokenMints(params: RawTokenMint): TokenMint[] {
		...
		if (Array.isArray(params)) return flatMap(this.toTokenMints)
		...
		if (Array.isArray(from) && to.length !== from.length)
			throw Error('Inconsistent mint sender length')
	...
	toTokenApprovals(params: RawTokenApproval): TokenApproval[] {
		...
		if (Array.isArray(params)) return flatMap(this.toTokenApprovals)
